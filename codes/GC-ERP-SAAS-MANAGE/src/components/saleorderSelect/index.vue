...
<div>
    ...
    <saleorder-select-dialog ref="selectDialog" @doSubmit="selectToInput" :limit="limit" :selectData="selectData"  :params="params"></saleorder-select-dialog>
</div>
</template>
<script>
import SaleorderSelectDialog from './saleorderSelectDialog.vue'
...
export default {
  ...
  watch: {
    ...
    selectData: {
      handler (newVal) {
        this.name = newVal.map(data => { return data.salesorderCode }).join(',')
      },
      immediate: false,
      deep: false
    }
  ...
  methods: {
    selectToInput (datas) {
      ...
      this.name = datas.map(data => { return data.salesorderCode }).join(',')
