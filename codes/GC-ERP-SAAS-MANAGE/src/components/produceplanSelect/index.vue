...
<div>
    ...
    <produceplan-select-dialog ref="selectDialog" @doSubmit="selectToInput" :limit="limit" :selectData="selectData"  :params="params"></produceplan-select-dialog>
</div>
</template>
<script>
import ProduceplanSelectDialog from './produceplanSelectDialog.vue'
...
export default {
  ...
  watch: {
    ...
    selectData: {
      handler (newVal) {
        this.name = newVal.map(data => { return data.produceplanCode }).join(',')
      },
      immediate: false,
      deep: false
    }
  ...
  methods: {
    selectToInput (datas) {
      ...
      this.name = datas.map(data => { return data.produceplanCode }).join(',')
