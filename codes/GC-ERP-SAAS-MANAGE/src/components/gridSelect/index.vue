<template>
  <div>
    <el-input :placeholder="placeholder" :disabled="disabled" :size="size" :readonly="true" style="line-hight:40px"
    ...
    <el-dialog :title="title" :close-on-click-modal="false" :append-to-body="true" v-dialogDrag width="75%"
      class="gridDialog" :visible.sync="visible">
      <el-row :gutter="15">
        <el-col :span="19">
          <el-form size="small" :inline="true" ref="searchForm" class="query-form" @keyup.enter.native="refreshList()"
            @submit.native.prevent>
            <el-form-item v-for="(search,index) in searchs" :key="index" v-show="search.show!=1" :prop="search.prop">
          ...
          <el-table :data="dataList" v-loading="loading" size="small" ref="gridTable" height="400px" border
            @selection-change="selectionChangeHandle" @sort-change="sortChangeHandle" style="width: 100%;">
            <el-table-column header-align="center" align="center" v-if="limit <= 1" width="50">
              <template slot-scope="scope">
                <el-radio :label="scope.row.id" :value="dataListAllSelections[0]&&dataListAllSelections[0].id"
            ...
            <template v-for="(column, index) in columns">
              <el-table-column v-if="column.label=='计量单位'" :key="index" :prop="column.prop" header-align="center"
                align="center" min-width="100px" :label="column.label">
                <template slot-scope="scope">
                  {{ $dictUtils.getDictLabel("sx_metering_unit", scope.row.singalUnit, '-')}}
                </template>
              </el-table-column>
              <el-table-column v-if="column.label!='计量单位' && column.label!='销售订单状态'&&!column.dictValue" :key="index" :prop="column.prop" header-align="center"
                align="center" min-width="100px" :label="column.label">
              </el-table-column>
              <el-table-column v-if="column.label=='销售订单状态'" :key="index" :prop="column.prop" header-align="center"
                align="center" min-width="100px" :label="column.label">
                <template slot-scope="scope">
                  {{ $dictUtils.getDictLabel("salesorder_state", scope.row.salesorderState, '-')}}
                </template>
              </el-table-column>

              <el-table-column v-if="column.dictValue" :key="index" :prop="column.prop" header-align="center"
                align="center" min-width="100px" :label="column.label">
                <template slot-scope="scope">
                  {{ $dictUtils.getDictLabel(column.dictValue, scope.row[column.prop], '-')}}
                </template>
              </el-table-column>
            </template>
    ...
    watch: {
        ...
        handler(newVal) {
          ...
          if (newVal) {
            newVal.split(',').forEach((value) => {
              if (value) {
                this.$http.get(`${this.queryEntityUrl}?${this.labelValue}=${value}`).then(({
                  data
                }) => {
      ...
      selectData: {
        handler(newVal) {
          ...
          this.name = this.dataListAllSelections.map(item => {
    ...
    methods: {
      ...
      refreshList() {
        ...
        this.searchs.forEach((search, index) => {
          if (search.show == 1) {
            searchForm[search.prop] = search.value
          } else {
      ...
      doSubmit() {
        ...
        this.visible = false
        // 多选数据-产品出库调用
        this.$emit('valueChanges', this.dataListSelections, this)
        ...
        // 单选数据调用
        this.$emit('valueChange', this.dataListSelections[0])
      },
      showSelectDialog() {
        this.visible = true
        this.init()
