<template>
  <div class="login-container">
    <div class="login-main animated bounceInDown">
      ...
      >
        <!-- <el-form-item prop="dba">
          <el-input
            type="text"
            placeholder="请输入租户号"
            v-model="inputForm.dba"
            autocomplete="on"
            name="account"
          >
...
export default {
  ...
  methods: {
    beglog() {
      this.loginService.beglog(this.inputForm);
    },
    // 提交表单
    login() {
      this.loginService
        .beglog(this.inputForm)
        .then(({ data }) => {
                ...
                .then(({ data }) => {
                  ...
                  this.userService.getMenus().then(({ data }) => {
                    localStorage.setItem(
                      "routerList",
                      JSON.stringify(data.routerList || "[]")
                    );
                    localStorage.setItem(
                      "allMenuList",
