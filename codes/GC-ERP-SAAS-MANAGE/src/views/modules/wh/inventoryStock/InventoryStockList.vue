        ...
        <vxe-table border="inner" auto-resize resizable height="auto" :loading="loading" size="small"
          ...
          <vxe-column type="expand" width="80">
            <template #content="{ row }">
              <el-tabs>
                <el-tab-pane label="存货库存详情表">
                  <el-table size="small" :data="row.InventoryStockDetailDTO" style="width: 100%" show-summary>
          ...
          <vxe-column field="inventory.inventoryName" sortable title="存货">
          </vxe-column>
          <vxe-column field="inventoryType" sortable title="存货类型">
          </vxe-column>
          <vxe-column field="inventory.inventoryCode" sortable title="存货编码">
          </vxe-column>
          <vxe-column field="inventory.model" sortable title="规格型号">
          </vxe-column>
          <vxe-column field="inventory.nventoryAttributeValue" sortable title="存货属性值">
          </vxe-column>
          ...
          <vxe-column field="inventory.avgPrice" sortable title="库存均价(元)">
          </vxe-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      refreshList() {
        this.loading = true
        this.inventoryStockService.list({
      ...
      detail({
        ...
        return new Promise(resolve => {
          this.inventoryStockService.queryById(row.inventory.id).then(({
            data
          }) => {
            this.dataList.forEach((item, index) => {
              if (item.inventory.id === row.inventory.id) {
                item.InventoryStockDetailDTO = data
              }
            })
            resolve()
          })
