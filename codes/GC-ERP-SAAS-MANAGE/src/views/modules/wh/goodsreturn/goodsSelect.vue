<template>
  <div>
    ...
    <el-dialog title="选择订单产品" :close-on-click-modal="false" :append-to-body="true" v-dialogDrag width="1000px"
        ...
        <el-col :span="19">
          ...
          <el-table :data="dataList" v-loading="loading" size="small" ref="gridTable" height="400px" border
            ...
            <el-table-column prop="inventory.inventoryName" header-align="center" align="center"
                    ...
                    <el-table-column prop="salesorderNum" header-align="center" align="center" show-overflow-tooltip
                      label="订单数量">
                    </el-table-column>
          </el-table>
  ...
  export default {
        ...
        handler(newVal) {
          ...
          if (newVal) {
            newVal.split(',').forEach((value) => {
              if (value) {
                this.$http.get(`/sales/salesorder/salesorder/detailById?id=${value}`).then(({
                  data
                }) => {
      ...
      selectData: {
        handler(newVal) {
          ...
          this.name = this.dataListAllSelections.map(item => {
            return item.inventory.inventoryName
          }).join(',')
        },
        immediate: false,
        deep: false
    ...
    methods: {
      ...
      refreshList() {
        this.loading = true
        this.$http({
          url: '/sales/salesorder/salesorder/detailList',
          method: 'get',
          params: {
            'current': this.pageNo,
            'size': this.pageSize,
      ...
      doSubmit() {
        ...
        // 单选数据调用
        this.$emit('valueChange', this.dataListSelections)
      },
      showSelectDialog() {
        this.visible = true
        this.init()
