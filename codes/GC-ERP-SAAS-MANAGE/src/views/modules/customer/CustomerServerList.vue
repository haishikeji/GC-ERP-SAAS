      ...
      <div style="height: calc(100% - 80px);">
        <vxe-table border="inner" auto-resize resizable height="auto" :loading="loading" size="small"
          ...
          <vxe-column field="serverState" sortable title="状态">
            <template slot-scope="scope">
              {{ $dictUtils.getDictLabel("cus_server_state", scope.row.serverState, '新建') }}
            </template>
          </vxe-column>
          <vxe-column fixed="right" align="center" width="250" title="操作">
            <template slot-scope="scope">
              <el-button v-if="hasPermission('customer:customerServer:view')" type="text" icon="el-icon-view" size="small"
                ...
                <el-button v-if="hasPermission('customer:customerServer:stat') && (scope.row.serverState == '0')"
                type="text" icon="el-icon-user" size="small" @click="accomplishOne(scope.row.id)">服务处理</el-button>
              <el-button v-if="hasPermission('customer:customerServer:del')" type="text" icon="el-icon-delete"
                size="small" @click="del(scope.row.id)">删除</el-button>
              <el-button v-if="hasPermission('customer:customerServer:stat') && (scope.row.serverState == '3')"
                type="text" icon="el-icon-user" size="small" @click="accomplish(scope.row.id)">归档</el-button>
            </template>
          </vxe-column>
        </vxe-table>
...
export default {
  ...
  methods: {
    ...
    accomplish(id) {
      this.customerServerService.statT(id).then(({
        data
      }) => {
        this.$message.success(data)
